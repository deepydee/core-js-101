...
const timeSpanToString = (startDate, endDate) => {
  ...
  const h = Math.trunc(diff / 3600000).toString().padStart(2, '0');
...
function angleBetweenClockHands(date) {
  ...
  const thetaH = 0.5 * (60 * hour + minute);
  const thetaM = 6 * minute;

  let delta = Math.abs(thetaH - thetaM);
  if (delta > 180) delta -= 180;

  const deltaRad = (delta * Math.PI) / 180;

  // return `Hour angle: ${thetaH}, Min angle: ${thetaM} delta: ${delta} deg, ${deltaRad} rad`;
  return deltaRad;
}
