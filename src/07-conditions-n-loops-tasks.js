 ...
 *
 */
const getFizzBuzz = (num) => ((num % 15 === 0) ? 'FizzBuzz'
...
function isTriangle(a, b, c) {
  const nums = [a, b, c];
  const max = nums.sort((x, y) => x - y).pop();

  return nums.reduce((sum, e) => sum + e, 0) > max;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  let str = '';
  const data = [a, b, isStartIncluded, isEndIncluded];
  const nums = data.splice(0, 2)
    .sort((x, y) => x - y);
 ...
 *   34143 => 34143
 */
const reverseInteger = (num) => [...num.toString()].reverse().join('');
...
function isCreditCardNumber(ccn) {
  const ccnStringified = ccn.toString();
  let sum = 0;
  let doubleDigit = false;

  for (let i = ccnStringified.length - 1; i >= 0; i--) {
    let digit = +ccnStringified[i];
...
function getDigitalRoot(num) {
  ...
  while (data.length > 1) {
    data = [...data.reduce((sum, e) => sum + +e, 0).toString()];
  }
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    if (index !== -1 && stack[stack.length - 1] !== str[i]) {
  ...
  // if all the opening and closing brackets are matched
  // and in the correct order, return true, otherwise return false
  return !stack.length;
}
...
function getCommonDirectoryPath(pathes) {
  const pathChunks = pathes.map((path) => path.split('/'));
  let commonDir = '';

  for (let i = 0; i < pathChunks[0].length; i++) {
    const chunk = pathChunks[0][i];

    for (let j = 0; j < pathChunks.length; j++) {
      if (pathChunks[j][i] !== chunk) {
        return commonDir;
      }
    }
...
function evaluateTicTacToePosition(position) {
  ...
  // Populate columns and diagonals arrays
  for (let i = 0; i < position.length; i++) {
    for (let j = 0; j < position[i].length; j++) {
      const element = position[i][j];

      if (element) {
        columns[j].push(element);
        if (i === j) diagonals[0].push(element);
        if (i + j === position.length - 1) diagonals[1].push(element);
      }
    }
  }
